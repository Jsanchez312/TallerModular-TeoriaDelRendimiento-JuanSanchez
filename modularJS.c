...
int main(int argc, char *argv[]) {
    //aqui se valida si se reciben por lo menos 2 argumentos
    if(argc <= 2){
    ...
    // se convierte a int el segundo argumento ingresado "N"
    int N = (int) atof(argv[1]);

    // se valida que la dimension sea mayor a 0
    if(N <= 0){
        printf("\nValores deben ser mayores que cero\n");
        return -1; // Sale con error
    };

    // aqui se reserva la memoria de forma dinamica para las matrices
    mA = (double *) malloc(N * N * sizeof(double));
    mB = (double *) malloc(N * N * sizeof(double));
    mC = (double *) calloc(N * N, sizeof(double));

    // se inicializan las matrices con valores predeterminados
    iniMatriz(N, mA, mB);

    // Se muestran las matrices iniciales si y solo si se cumple que la matriz sea de tamaño menor a 9
    imprMatrices(N, mA);
    imprMatrices(N, mB);

    InicioMuestra();

    // aqui se hace la multiplicacion de las matrices
    multimatrizclasica(mA, mB, mC, N);

    // se calcula el tiempo de ejecucion y el tiempo final
    FinMuestra();
    ...
    // Se libera la memoria dinámica reservada
    free(mA);
    free(mB);
    free(mC);
